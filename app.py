...
# Path to the final CSV produced by the NLP pipeline
DATA_PATH = "data/classified_emails.csv"
# Path to the sentence embeddings used for semantic search
EMBEDDINGS_PATH = "data/embeddings.npy"
# Sentence-transformer model used to embed emails and user queries
EMBEDDER_MODEL = "all-MiniLM-L6-v2"
# Same threshold used in Stage 1 to decide if an email is problematic
BINARY_THRESHOLD = 0.35

#color palettte used for graphs
PALETTE = {
    "problematic": "#C0392B",
    "safe":        "#2E5FA3",
    "neutral":     "#7F8C8D",
...
# Manual alias groups used to merge different variants of the same person's name
PERSON_ALIAS_GROUPS = {
    "Donald Trump": {
        "donald",
        "trump",
...
def normalize_persons(persons: list[str]) -> list[str]:
    """
    Normalize person names using explicit alias groups first, then fall back to cleaned title-cased names.
    """
    normalized = []
    ...
    for person in persons:
        ...
        # First try to map short / partial names to a canonical full name
        if key in PERSON_ALIAS_LOOKUP:
            normalized.append(PERSON_ALIAS_LOOKUP[key])
        else:
            #Otherwise appy standard title-casing cleanup
            normalized.append(_normalize_name(raw))

    return normalized

# Pages
# Defines page title, icon, and layout when the app opens
st.set_page_config(
...
# Small custom CSS to improve the dashboard look:
# - larger metric numbers
# - clearer tab styling
# - styled email detail box
st.markdown(
    """
    <style>
...
def load_data() -> pd.DataFrame:
    # Load the final dataset generated by the pipeline
    df = pd.read_csv(DATA_PATH)

    # Some columns were saved as string versions of Python lists in the CSV
    # We convert them back into real lists so the app can use them normally
    ...
    # Create display-friendly helper columns if they are missing.
    # These make it easier to show readable values in the dashboard UI.
...
def load_search_index():
    ...
    # Load the same embedding model used during preprocessing
    embedder   = SentenceTransformer(EMBEDDER_MODEL)
    
    # Load the saved email embeddings from disk
    embeddings = np.load(EMBEDDINGS_PATH).astype("float32")

    # Create a FAISS index for efficient vector similarity search
...
def make_risk_pie(df: pd.DataFrame) -> go.Figure:
    # Count problematic vs safe emails
...
def make_offense_bar(df: pd.DataFrame) -> go.Figure:
    # Count offense labels only for emails already flagged as problematic
    offense_counts: dict = {}
    ...
    # If there are no offense labels, return an empty placeholder chart
    ...
    # Horizontal bar chart ranking offense types by frequency
...
def make_score_hist(df: pd.DataFrame) -> go.Figure:
    # Histogram of Stage 1 probability scores
    # Helps visualize where emails fall relative to the classification threshold
...
def make_top_persons_bar(df: pd.DataFrame) -> go.Figure:
    ...
    # Normalize and merge equivalent names before counting mentions
    ...
    # Horizontal bar chart of most frequently mentioned people
    fig = px.bar(
        x=top.values,
        y=top.index,
        orientation="h",
...
def keyword_search(
    ...
    if active:
        if match_mode == "AND":
            ...
            # Start from no emails and keep those containing at least one keyword
...
# Optional filtering by risk status
    ...
    # Optional filtering by offense category
    ...
    # Sort by problematic score so the most suspicious results appear first
...
def semantic_search(
    ...
    # Normalize query embedding so inner product behaves like cosine similarity
